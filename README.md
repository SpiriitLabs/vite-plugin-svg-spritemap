...
If you are using Sass, Less or Stylus: you can optimize the export by only generate one mixin on one instance with `styles.include` set to `['variables']` or/and use the `styles.names` object.
...
| include     | boolean or `['mixin', 'variables']` or `['bg', 'mask', 'bg-frag']` or `undefined` | By default, it will include `['mixin', 'variables']` for SCSS/Less/Stylus and `['bg', 'mask', 'bg-frag']`. This allowing you tp choose what to include  |
