...
In the example below, this will generate only the background data uri and not all classes (mask and fragment) like done by default.
...
For SCSS, Less and Stylus, you can disable the mixin by passing `false` to `styles.includeMixin`.
...
If you are using Sass, Less or Stylus: you can optimize the export by only generate one mixin on one instance with `styles.includeMixin` set to `false` or/and use the `styles.names` object.
