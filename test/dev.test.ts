...
describe('dev server', () => {
  ...
  it('has HMR', async () => {
    ...
    // Create test
    await writeFile(getPath('./fixtures/basic/hmr/vite.svg'), viteLogo('red'))
    ...
    // Update test
    await writeFile(getPath('./fixtures/basic/hmr/vite.svg'), viteLogo('#000'))
    const revertResponse = await page.waitForResponse(response =>
      response.url().includes('/spritemap.css'),
    )
    const revertText = await revertResponse.text()
    expect(revertText).toMatchSnapshot()

    expect(text).not.toBe(revertText)

    // Delete test
    await unlink(getPath('./fixtures/basic/hmr/vite.svg'))
    // const svgDeleted = await page.waitForResponse(response =>
    //   response.url().includes('__spritemap'),
    // )
    // const svgDeletedContent = await svgDeleted.text()
    // expect(svgDeletedContent.includes('#vite')).toBeFalsy()
