  ...
  server = await createServer({
    ...
    plugins: [
      VitePluginSvgSpritemap(getPath('./project/svg/*.svg'), {
        styles: getPath('./project/styles/spritemap.scss')
      })
    ]
  })
  await server.listen()
...
describe('dev server', () => {
  ...
  it('has HMR', async () => {
    let refreshSpritemap = 0
    page.on(
      'response',
      response =>
        (refreshSpritemap += response.url().includes('__spritemap__') ? 1 : 0)
    )
    ...
    writeFile(getPath('./project/svg/vite.svg'), viteLogo('#000'))
    await page.waitForResponse('**/__spritemap__**')

    writeFile(getPath('./project/svg/vite.svg'), viteLogo('currentColor'))
    await page.waitForResponse('**/__spritemap__**')

    expect(refreshSpritemap).toBe(3)
  })
})
