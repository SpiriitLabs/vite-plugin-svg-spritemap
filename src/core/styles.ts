...
export class Styles {
  ...
  private async insert(insert: string): Promise<string> {
    ...
    // a template owns its layout through a `__data__` slot, so sass keeps its `@use`
    // loads at the top of the file. Without a slot (css, no mixin) data is all there is
    if (!template.includes('__data__'))
      return `${doNotEditThisFile + insert}\n${template}`

    return doNotEditThisFile + template.split('__data__').join(insert.trim())
  }
